...
export default function ExamPreviewCanvas() {
  ...
  return (
    <GlassCard title="A4 預覽畫布" description="這裡會先做前端版面預覽，之後再交給 @react-pdf/renderer 或後端服務輸出正式 PDF。">
      <div className="mx-auto w-full max-w-[460px] max-h-[85vh] overflow-auto rounded-[32px] border border-white/10 bg-white p-6 text-slate-900 shadow-2xl">
        ...
        <div className="mt-5 grid gap-4">
          {crops.length === 0 ? (
            ...
            crops.map((crop, index) => (
              <div key={crop.id} className="rounded-[16px] border border-slate-200 p-3">
                ...
                <div className="mt-2 rounded-xl border border-slate-200 bg-white">
                  <img
                    ...
                    style={{ maxHeight: `${Math.min(getPreviewHeight(crop), 240)}px` }}
                  />
                </div>
              </div>
            ))
